...
# Dot-source from 2 / 4 / 5. Keeps Bandcamp+DistroKid login; clears locks/caches/legacy folders.

function Get-DkBcChromeProfileDir {
  Join-Path $env:LOCALAPPDATA "DK-BC-Uploader\chrome-debug-profile"
}

function Ensure-DkBcChromeProfileWritable {
  $dir = Get-DkBcChromeProfileDir
...
function Stop-DkBcDebugChrome {
  $markers = @(
    "DK-BC-Uploader\chrome-debug-profile",
...
function Clear-DkBcChromeLocks {
  $dir = Get-DkBcChromeProfileDir
...
function Remove-DkBcEphemeralCache {
  $ephemeral = @(
  ...
  foreach ($base in @((Get-DkBcChromeProfileDir), (Join-Path (Get-DkBcChromeProfileDir) "Default"))) {
...
function Remove-DkBcLegacyLocalSecrets {
  param([string[]]$Roots)
  foreach ($r in $Roots) {
    if (-not $r) { continue }
...
function Invoke-DkBcSessionCleanup {
  param(
    [string[]]$AppRoots = @($PSScriptRoot),
    [switch]$RemoveLogin
  )
  $n = Stop-DkBcDebugChrome
  Start-Sleep -Seconds 1
  Clear-DkBcChromeLocks
  if ($RemoveLogin) {
    $root = Join-Path $env:LOCALAPPDATA "DK-BC-Uploader"
    ...
    Remove-DkBcEphemeralCache
    Ensure-DkBcChromeProfileWritable | Out-Null
  }
  Remove-DkBcLegacyLocalSecrets -Roots $AppRoots
  return $n
}
